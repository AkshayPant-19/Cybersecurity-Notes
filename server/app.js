...
const sections = [
  ...
  { id: 'meltdown-spectre', title: 'Meltdown and Spectre', icon: '🧬', category: 'HARDWARE AND SOFTWARE VULNERABILITIES',
    content: '<h3>Meltdown & Spectre</h3><ul><li><strong>Meltdown</strong> - Breaks the isolation between user applications and the OS, allowing a program to access memory from a given system</li><li><strong>Spectre</strong> - Tricks other applications into accessing arbitrary locations in their memory, leaking data handled by other applications</li><li><strong>Rowhammer</strong> - Exploits dense memory cells by repeatedly accessing (hammering) rows to cause bit flips in adjacent rows</li><li>All are <strong>side-channel</strong> attacks exploiting CPU hardware vulnerabilities</li></ul>',
    sources: [{name:'MeltdownAttack.com',url:'https://meltdownattack.com/'},{name:'Spectre Attacks',url:'https://spectreattack.com/'},{name:'Rowhammer - Wikipedia',url:'https://en.wikipedia.org/wiki/Row_hammer'}],
    quiz: [{q:'Which vulnerability breaks OS-user isolation to read kernel memory?',o:['Spectre','Rowhammer','Meltdown','SQL Injection'],a:2},{q:'Spectre leaks data handled by:',o:['The kernel','Other applications','The network','Storage devices'],a:1},{q:'Rowhammer causes bit flips by:',o:['Repeatedly accessing memory rows','Overheating the CPU','Writing to disk','Sending network packets'],a:0}] },
  { id: 'software-vulns', title: 'Software Vulnerabilities', icon: '🐛', category: 'HARDWARE AND SOFTWARE VULNERABILITIES',
    content: '<h3>Software Vulnerabilities</h3><ul><li><strong>Buffer Overflow</strong> - Buffers are memory areas allocated to an application. A vulnerability occurs when data is written beyond the limits of a buffer, potentially overwriting adjacent memory.</li><li><strong>Non-Validated Input</strong> - Programs often require data input, but this incoming data could have malicious content, designed to force the program to behave in an unintended way.</li><li><strong>Race Conditions</strong> - A situation where the output of an event depends on ordered or timed outputs. When timing is exploited, an attacker can manipulate the outcome.</li></ul>',
    sources: [{name:'OWASP - Buffer Overflow',url:'https://owasp.org/www-community/vulnerabilities/Buffer_Overflow'},{name:'OWASP - Input Validation',url:'https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html'},{name:'CWE - Race Conditions',url:'https://cwe.mitre.org/data/definitions/362.html'}],
    quiz: [{q:'What happens in a buffer overflow?',o:['Data read too slowly','Data written beyond buffer limits','Data encrypted incorrectly','Data deleted'],a:1},{q:'Non-validated input can cause:',o:['Faster execution','Unintended program behavior','Memory savings','Better UX'],a:1},{q:'Race conditions depend on:',o:['Password strength','Network speed','Ordered or timed outputs','File size'],a:2}] }
];
